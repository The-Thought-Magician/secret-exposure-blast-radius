    ...
    <main className="min-h-screen bg-zinc-950 text-zinc-100">
      <nav className="border-b border-zinc-800 px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3 text-sm">
          ...
          <Link href="/auth/sign-up" className="bg-pink-600 hover:bg-pink-500 text-white px-4 py-2 rounded-lg font-medium">Get Started</Link>
        </div>
      </nav>
      ...
      <section className="max-w-4xl mx-auto px-6 py-20 text-center">
        ...
        <div className="mt-12 grid gap-6 md:grid-cols-2">
          {/* Free plan */}
          <div className="rounded-xl border border-pink-900/60 bg-zinc-900/60 p-8 text-left">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-bold">Free</h2>
              <span className="rounded-full border border-pink-900/60 bg-pink-950/30 px-2 py-0.5 text-xs font-medium text-pink-300">Current plan</span>
            </div>
            ...
            <ul className="mt-6 space-y-2 text-sm text-zinc-300">
              {freeFeatures.map((f) => (
                <li key={f} className="flex gap-2"><span className="text-pink-500">✓</span><span>{f}</span></li>
              ))}
            </ul>
            <Link href="/auth/sign-up" className="mt-8 block rounded-lg bg-pink-600 hover:bg-pink-500 px-4 py-3 text-center font-semibold text-white">
              Start free
            </Link>
          </div>
        ...
        <p className="mt-12 text-sm text-zinc-500">
          No credit card required. <Link href="/" className="text-pink-400 hover:text-pink-300">Back to home</Link>
        </p>
      </section>
    </main>
  )
